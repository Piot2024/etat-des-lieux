...
function handleRoomClick(e){
  ...
  let r=a.rooms.find(x=>x.id===roomId);

  if(act==='move-up'){
    let idx=a.rooms.findIndex(x=>x.id===roomId);
    if(idx>0){
      let tmp=a.rooms[idx-1];
      a.rooms[idx-1]=a.rooms[idx];
      a.rooms[idx]=tmp;
      save();
      renderRooms();
      showTab('rooms');
  ...
  if(act==='add-element'&&r){
    r.elements.push(newElement(''));
    save();
    renderRooms();
    showTab('rooms');
    return;
  ...
  if(act==='remove-room'){
    a.rooms=a.rooms.filter(x=>x.id!==roomId);
    save();
    renderRooms();
    showTab('rooms');
    return;
  ...
  if(act==='remove-element'&&r){
    r.elements=r.elements.filter(x=>x.id!==e.target.dataset.elid);
    save();
    renderRooms();
    showTab('rooms');
    return;
